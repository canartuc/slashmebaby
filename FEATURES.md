...
These features are required for the initial public release on Chrome Web Store and Firefox Add-ons.
...
| F02 | Tab Search & Switch | Fuzzy search across all open tabs in all windows. Results show tab title, URL, and favicon. Pressing Enter switches to the selected tab.  Hibernated (discarded/Memory-Saver) tabs stay searchable, show a "zzz" sleep badge on both surfaces (including pinned tiles, announced to screen readers), and are woken when switched to — an explicit reload on Chrome; Firefox restores them natively. | Complete |
...
| F04 | History Search | Fuzzy search across the last 1,000 browsing history items (cached in the background, refreshed every 5 minutes). Available on both surfaces (a History group appears once a query is typed; never in the empty tree state) — the overlay and the toolbar popup run the identical palette. Results show title and URL. Pressing Enter navigates the current tab to the URL. | Complete |
| F05 | Recency-Weighted Scoring | Retained in the background SEARCH pipeline (a stable message API with no shipped UI consumer since the popup unification): `finalScore = (1 - fuseScore) * 0.6 + recencyScore * 0.4`, exponential-decay recency with half-lives Tabs 2h, Bookmarks 168h, History 24h. Both palette surfaces rank by Fuse match quality. | Complete |
| F06 | Grouped Results | Results are displayed in labelled sections, identically on both surfaces: Open Tabs → Bookmarks → History, with a Navigate ("Go to URL") row last when applicable. Each group is capped by the Results Per Group setting (3/5/8, default 5); there is no separate global cap. Sections only appear if they have matching results. | Complete |
| F07 | Keyboard Navigation | Arrow keys move between items across group boundaries (wrapping at the ends). Tab / Shift+Tab jump to the first item of the next / previous section (result groups in search mode, top-level folders in the tree view), wrapping across the list; in single-section lists (e.g. `>` action mode) they fall back to stepping one item. Enter executes the selected item. Escape dismisses. Backspace edits the query normally and never dismisses either surface. | Complete |
| F08 | Smart Suggestions | Retired as a distinct popup surface: the empty state on BOTH surfaces is the jump view — pinned-tab grid, open-tab grid, bookmark tree, and action chips (F25). The background SMART_SUGGESTIONS handler remains as a stable message API with no shipped UI consumer. | Superseded |
...
These features improve the product experience but are not required for launch. Target: first minor release post-launch.
...
| F20 | Icon Click & Popup | Clicking the toolbar icon on normal pages opens the in-page overlay (per-tab popup routing: the background clears the per-tab popup on injectable tabs so clicks fire `action.onClicked`). On restricted pages (`chrome://`, `about:`, new tab, Web Store) the icon opens the popup, which renders the same CommandBar palette as the overlay and opens in jump mode identical to it (labels pressable on entry, '/' for typed search) in a 720×540 window. Surface parity is pinned by dedicated unit, e2e, and pixel-diff test suites. | Complete |
