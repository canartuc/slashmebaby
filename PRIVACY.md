...
SlashMeBaby is a browser extension that provides a keyboard-driven command palette for searching your open tabs, bookmarks, and browsing history, and for running tab-management actions. This policy describes what data the extension accesses, how it is used, and — most importantly — what never leaves your browser.

## Summary

SlashMeBaby processes everything on your device. It does not collect, transmit, sell, or share any data. There are no accounts, no analytics, no telemetry, no advertising, and no remote code. One technical nuance about rendering the palette on web pages is described under "On-page rendering" below.
...
- **`chrome.storage.sync`** — your UI settings only: keyboard shortcut, palette position, theme, maximum results per group, favicon display toggle, and which search sources (tabs, bookmarks, history) are enabled. If you are signed into your browser, the browser may sync these settings across your devices; that syncing is handled entirely by your browser vendor under its own policy.
- **`chrome.storage.local`** — your onboarding progress only (which tutorial step you have completed), so the first-run tutorial does not reappear.

Nothing else is stored. No browsing data, no search queries, no personal information.
