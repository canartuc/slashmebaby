...
| ID | Priority | Description |
|----|----------|-------------|
| TS-009 | P0 | Pressing the configured shortcut (default Alt+Space) opens the command bar overlay |
...
| TS-015 | P0 | Focus returns to the previously focused element on the host page after closing |

### 2.2 All Four Shortcut Options

| ID | Priority | Description |
|----|----------|-------------|
| TS-016 | P0 | Alt+Space opens the command bar when configured as the activation shortcut |
| TS-017 | P0 | Ctrl+Shift+L opens the command bar when configured as the activation shortcut |
| TS-018 | P0 | Ctrl+. opens the command bar when configured as the activation shortcut |
| TS-019 | P0 | Ctrl+/ opens the command bar when configured as the activation shortcut |
| TS-020 | P1 | Custom shortcut set via chrome://extensions/shortcuts opens the command bar |
...
| ID | Priority | Description |
|----|----------|-------------|
| TS-026 | P0 | Opening the command bar with no query shows smart suggestions immediately |
...
| TS-035 | P0 | Typing a query returns fuzzy-matched actions by name |
...
| TS-039 | P0 | Clearing the query returns to the smart suggestions empty state |

### 3.3 Grouped Results

| ID | Priority | Description |
|----|----------|-------------|
| TS-040 | P0 | Results are displayed in groups in order: Tabs, Bookmarks, History, Actions |
...
| TS-043 | P0 | Each group shows a maximum of 5 results |
| TS-044 | P0 | Total visible results across all groups do not exceed 15 |
...
| TS-054 | P0 | Typing ">" shows only the Actions group |
...
| TS-062 | P0 | Tab key jumps forward to the next group header |
| TS-063 | P0 | Shift+Tab key jumps backward to the previous group header |
...
| TS-072 | P0 | "Close Other Tabs" action closes all tabs in the current window except the current one |
...
| TS-082 | P0 | "Recently Closed" action opens a sub-list showing up to 10 recently closed tabs |
| TS-083 | P0 | Selecting a tab from the "Recently Closed" sub-list restores that tab |
| TS-084 | P0 | "Go to URL" action navigates the current tab to the URL typed in the query |
| TS-085 | P0 | "Close All Duplicates" action closes all tabs with duplicate URLs, keeping one per URL |
| TS-086 | P0 | "Sort Tabs by Domain" action reorders all open tabs grouped by their domain |
| TS-087 | P0 | "Settings" action opens the SlashMeBaby settings page |
...
| TS-091 | P0 | The overlay closes after any action is executed successfully |
...
| ID | Priority | Description |
|----|----------|-------------|
| TS-095 | P1 | Settings page shows all four shortcut options: Alt+Space, Ctrl+Shift+L, Ctrl+., Ctrl+/ |
...
| TS-098 | P1 | The previously configured shortcut no longer opens the command bar after changing |
...
| TS-120 | P1 | Step 2 prompts the user to press their chosen shortcut to activate the live command bar |
...
| TS-123 | P1 | Step 4 includes a link to the settings page |
| TS-124 | P1 | Completing step 4 marks onboarding as complete in chrome.storage.local |
| TS-125 | P1 | A progress indicator shows the current step number and total steps (e.g., "2 of 4") |
...
| TS-150 | P0 | Firefox MV2/MV3 manifest is valid and accepted by the Firefox Add-ons review |
| TS-151 | P0 | WXT browser namespace polyfill resolves API differences transparently |
...
| TS-176 | P0 | "Recently Closed" sub-list shows an empty state when no tabs have been closed this session |
